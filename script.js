...
function startAnimation() {
    ...
    fetchAndUpdateQuotes(); // 立即执行一次
    animationInterval = setInterval(fetchAndUpdateQuotes, 15000); // 每15秒轮询
}
...
async function fetchAndUpdateQuotes() {
    ...
    try {
        // *** 关键修正：向同一个API发送带mode=quotes的请求 ***
        const res = await fetch('/api/stocks?mode=quotes');
        ...
        latestQuotes.forEach(quote => {
            ...
            if (stockToUpdate && typeof quote.dp === 'number' && stockToUpdate.change_percent !== quote.dp) {
                stockToUpdate.change_percent = quote.dp;
                updateStockDOM(stockToUpdate);
                updated = true;
            }
        });
        if (updated) console.log('Heatmap updated with latest quotes.');
...
function generateTreemap(data, container, groupIntoSectors) {
    container.innerHTML = '';
    ...
    if (groupIntoSectors) {
        ...
        itemsToLayout = Object.entries(stocksBySector).map(([sectorName, sectorData]) => ({ name: sectorName, value: sectorData.total_market_cap, items: sectorData.stocks.map(s => ({ ...s, value: s.market_cap })) })).sort((a, b) => b.value - a.value);
    } else {
...
window.addEventListener('resize', () => {
    ...
    window.resizeTimeout = setTimeout(() => {
        ...
        if (fullMarketData) {
            ...
            if(container) {
                const dataToRender = sector ? fullMarketData.filter(s => s.sector === decodeURIComponent(sector)) : fullMarketData;
                generateTreemap(dataToRender, container, !sector);
            }
        }
    }, 250);
});