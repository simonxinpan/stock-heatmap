...
// --- 3.  **小面积方块**: 只显示最重要的信息（股票代码），或者甚至只显示颜色，不显示任何文字。

这个策略将通过JavaScript在渲染时动态判断，并为元素添加不同的CSS类来实现。

---

### 行动计划：最后的UI精 polish

这一轮，我们将专注于 `script.js` 和 `style.css` 的精细打磨。

#### `script.js` (引入智能内容展示逻辑)

我们将修改 `createStockElement` 函数，让它在创建股票方块时，根据计算出的面积，为其路由系统 (保持不变) ---
    ...
    try {
        ...
        appContainer.innerHTML = `
            ...
            <footer class="legend">
                ...
                <div class="legend-item"><div class="legend-color-box loss-medium"></div><span>-1%</span>添加不同的CSS类。

**请用以下完整代码覆盖 `script.js` 文件：**
```javascript
const appContainer = document.getElementById('app-container');
let fullMarketData = null; 
...
async function renderHomePage() {
    try {
        ...
        appContainer.innerHTML = `<div class="loading-indicator">${</div>
                <div class="legend-item"><div class="legend-color-box flat"></div><span>0%</span></div>
                <div class="legend-item"><div class="legend-color-box gain-medium"></div><span>+1%</span></div>
                <div class="legend-item"><div class="legend-color-box gain-strong"></div><span>> +2%</span></div>
            </footer>
...
function generateTreemap(allStocks, container) {
    // ... (generateTreemap函数的前半部分保持不变) ...
    // ... (groupDataBySector, layout, squarify等函数都保持不变) ...
    ...
    function layout(items, x, y, width, height, parentEl, isSectorLevel) {
        ...
        if (isSectorLevel) {
            ...
            const titleHeight = 28; // 标题栏高度
            const contentWidth = width - 4; // 减去边框
            const contentHeight = height - titleHeight - 2;

            if (contentWidth > 0 && contentHeight > 0) {
                 layout(currentItem.items, 2, titleHeight, contentWidth, contentHeight, itemEl, false);
            }
        } else {
...
// **核心修改：引入ResizeObserver**
function createStockElementerror.message}</div>`;
    }
}
...
function generateTreemap(allStocks, container) {
    ...
    // 启动布局
    layout(sectors, 0, 0, totalWidth, totalHeight, true, container);

    function layout(items, x, y, width, height, isSectorLevel, parentElement) {
        ...
        if (isSectorLevel) {
            ...
            const titleHeight = (width > 200 && height > 100) ? 30 : 0;
            if(titleHeight === 0) titleEl.style.display = 'none';

            layout(currentItem.items, 0, titleHeight, width, height - titleHeight, false, itemEl);
        ...
        itemEl.style.left = `${x}px`;
        itemEl.style.top = `${y}px`;
        itemEl.style.width = isHorizontal ? `${width * itemProportion}px` : `${width}px`;
        itemEl.style.height = isHorizontal ? `${height}px` : `${height * itemProportion}px`;
        
        parentElement.appendChild(itemEl);
        
        if (isHorizontal) {
            let processedWidth = width * itemProportion;
            layout(items.slice(1), x + processedWidth, y, width - processedWidth, height, isSectorLevel, parentElement);
        } else {
            let processedHeight = height * itemProportion;
            layout(items.slice(1), x, y + processedHeight, width, height - processedHeight, isSectorLevel, parentElement);
        }
    }
}
...
function createStockElement(stock, width, height) {
    ...
    if (area > 8000) {
        stockDiv.classList.add('detail-full'); // 显示全部
    } else if (area > 3000) {
        stockDiv.classList.add('detail-medium'); // 显示代码和涨跌幅
    } else {
        stockDiv.classList.add('detail-small'); // 只显示代码
    }
...
async function renderStockDetailPage(symbol) {
                ...
                <main class="main-content">
                    <div class="stock-header">
                        ...
                        <div class="stock-price-info">
                            <div class="current-price">${quote(stock) {
    const stockLink = document.createElement('a');
    stockLink.className = 'treemap-stock';
    stockLink.href = `/?page=stock&symbol=${stock.ticker}`;
    stockLink.onclick = (e) => navigate(e, stockLink.href);
    ...
    stockDiv.className = `stock ${getColorClass(change)}`;
    stockDiv.innerHTML = `
        <span class="stock-sector-label">${stock.sector}</span>
        <span class="stock-ticker">${stock.ticker}</span>
        <span class="stock-name-zh">${stock.name_zh}</span>
        <span class="stock-change">${change >= 0 ? '+' : ''}${change ? change.toFixed(2) : '0.00'}%</span>`;
    stockLink.appendChild(stockDiv);
    ...
    const observer = new ResizeObserver(entries => {
        for (let entry of entries) {
            ...
            // 移除旧的尺寸类
            target.classList.remove('is-tiny', 'is-small', 'is-medium', 'is-large');

            // 根据尺寸添加新的类
            if (width < 60 || height < 40) {
                target.classList.add('is-tiny');
            } else if (width < 100 || height < 60) {
                target.classList.add('is-small');
            } else if (width < 150 || height < 90) {
                target.classList.add('is-medium');
            } else {
                target.classList.add('is-large');
            }
        }
    });

    observer.observe(stockDiv); // 监视 .stock 元素
